...
raw_numbers =   """4347    3350    196 162 233 4932    4419    3485    4509    4287    4433    4033    207 3682    2193    4223
...
def processToArray(raw_data):
    x, y = 0, 0
    processed_data = [[""]]

    for n in range(0, len(raw_data)):
        if raw_data[n] is " ":
            if len(processed_data[x])==y+1:
                if processed_data[x][y] is not "":
                    y += 1                    
                    processed_data[x].extend([""])
        elif raw_data[n] is "\n":
            processed_data.append([""])
            x += 1
            y = 0
        else:
            processed_data[x][y] += (raw_data[n])
